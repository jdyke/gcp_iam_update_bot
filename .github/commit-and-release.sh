...
if [[ ${CHANGES} -eq 1 ]]; then
  ...
  RELEASE_BODY="$(printf '{"tag_name": "%s","target_commitish": "main","name": "GCP IAM Update Detected on ${NEW_TAG}","body": "%s","draft": false,"prerelease": false}' $NEW_TAG $NEW_TAG "$RELEASE_TEXT")"
  # Push release
